...
description: Sequences the Vibe Innovation Framework phases (entry diagnostic, phases 0 to 5, status reporting). Thin agent that delegates all process logic to the existing skills and to ${CLAUDE_PLUGIN_ROOT}/docs/orchestrator.md.
tools:
  - Read
  - Edit
  - Write
...
1. The full process specification, including gate decisions, loop-back limits, and persona handoffs, lives in `${CLAUDE_PLUGIN_ROOT}/docs/orchestrator.md`.
2. The Innovation Canvas Document (ICD) specification lives in `${CLAUDE_PLUGIN_ROOT}/docs/innovation_canvas_document.md` and is validated by `${CLAUDE_PLUGIN_ROOT}/schemas/innovation_canvas.schema.json`.
3. The TRL scale and artifact maturity progression live in `${CLAUDE_PLUGIN_ROOT}/docs/trl_specification.md`.
